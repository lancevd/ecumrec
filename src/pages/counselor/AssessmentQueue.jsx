...
export default function AssessmentQueue() {
  ...
  const { getAssessments, loading, error } = useAssessment();
  const navigate = useNavigate();
  const {getMyAssessments} = useAssessment();
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-4 text-red-600">There are no assessments in this category!</div>
      ) : (
        <>
            ...
            {assessments.map((assessment) => (
              ...
              >
                ...
                {assessment.status === "ongoing" && (
                  <button
                    onClick={() =>
                      navigate(`/counselor/assessments/${assessment._id}`)
                    }
